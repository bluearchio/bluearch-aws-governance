...
def test_release_jobs_verify_final_archives_before_sbom_and_publish():
    ...
    notarize_index, _ = _named_step(jobs["macos"], "Codesign and notarize macOS asset")
...
def test_macos_verifier_rejects_unsigned_final_archive(tmp_path):
    unsigned = tmp_path / "bluearch-aws-governance"
    unsigned.write_text("#!/bin/sh\necho 0.2.4\n", encoding="utf-8")
    unsigned.chmod(0o755)
    archive = tmp_path / "bluearch-aws-governance-macos-arm64.zip"
    subprocess.run(
        ["ditto", "-c", "-k", "--keepParent", os.fspath(unsigned), os.fspath(archive)],
        check=True,
    )

    result = subprocess.run(
        ["bash", os.fspath(MACOS_VERIFIER), os.fspath(archive), "bluearch-aws-governance"],
        cwd=ROOT,
        capture_output=True,
        text=True,
        check=False,

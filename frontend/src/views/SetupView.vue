...
function templateNameFor(component: string): string | undefined {
  return componentTemplateMap.value[component]
}

async function loadComponentTemplateMap() {
  try {
...
const allInfraComponents = computed<InfraComponent[]>(() => {
  ...
  if (ss) {
    ...
    components.push({
      ...
      templateName: templateNameFor('cross-account'),
    })
  } else {
    components.push({
      key: 'cross-account',
  ...
  if (mgmt) {
    ...
    components.push({
      ...
      action: 'manage',
      actionRoute: '/setup/multi-account',
      templateName: templateNameFor('management-resources'),
    })
  } else {
    components.push({
      key: 'management',
  ...
  if (role) {
    ...
    components.push({
      ...
      templateName: templateNameFor('assume-role'),
    })
  } else {
  ...
  if (cur) {
    ...
    components.push({
      ...
      versionOutdated: !!(localVer && (!ver || ver !== localVer)),
      action: 'delete',
      templateName: templateNameFor('cost-reports'),
    })
  } else {
    components.push({
      key: 'cur',
  ...
  if (etActive || etHasQueues) {
    ...
    components.push({
      ...
      versionOutdated: false,
      action: 'delete',
      templateName: templateNameFor('event-tracking'),
    })
...
async function downloadIamPolicy() {
  ...
  link.href = url
  link.download = 'bluearch-iam-policy.json'
...
/* Self-hosted Deploy */
.self-hosted-deploy-idle {
  padding: 1.5rem;
}

.self-hosted-deploy-desc {
  font-size: 0.9rem;
  color: var(--text-color-secondary);
  margin: 0 0 0.75rem;
  line-height: 1.5;
